...
if (! function_exists('currencyDollarExchange')) {
    function currencyDollarExchange($amount, $from = null, $to = null, $round = 10)
    {
        $to = $to ? $to : config('templatecookie.currency', 'USD');
        $toRate = Currency::whereCode($to)->first()->rate;
        $rate = round($amount / $toRate,10 );
        return $rate;
    }  
}
