<!-- Hero Section -->
<section class="hero-section py-5 mb-5 position-relative overflow-hidden">
    <div class="gradient-bg position-absolute w-100 h-100" style="opacity: 0.1;"></div>
    <div class="container position-relative">
        <div class="row align-items-center min-vh-75">
            <div class="col-lg-6" data-aos="fade-right">
                <h1 class="display-4 fw-bold mb-4">
                    Ваша реклама - наш успіх!
                    <span class="text-gradient d-block">AdBoard Pro</span>
                </h1>
                <p class="lead mb-4">
                    Професійна рекламна компанія та сучасна дошка оголошень. 
                    Ми допоможемо вашому бізнесу досягти нових висот та знайти цільову аудиторію.
                </p>
                <div class="d-flex flex-wrap gap-3">
                    <a href="<?php echo SITE_URL; ?>/ads" class="btn gradient-bg text-white btn-lg hover-scale">
                        <i class="fas fa-search me-2"></i>Переглянути оголошення
                    </a>
                    <a href="<?php echo SITE_URL; ?>/services" class="btn btn-outline-primary btn-lg hover-scale">
                        <i class="fas fa-cogs me-2"></i>Наші послуги
                    </a>
                </div>
            </div>
            <div class="col-lg-6" data-aos="fade-left">
                <div class="position-relative">
                    <img src="https://via.placeholder.com/600x400/667eea/ffffff?text=AdBoard+Pro" 
                         alt="AdBoard Pro" 
                         class="img-fluid rounded shadow-lg animate-float">
                    <div class="position-absolute top-0 start-0 w-100 h-100 gradient-bg rounded" style="opacity: 0.1;"></div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Features Section -->
<section class="features-section py-5">
    <div class="container">
        <div class="text-center mb-5" data-aos="fade-up">
            <h2 class="display-5 fw-bold mb-3">Чому обирають нас?</h2>
            <p class="lead text-muted">Переваги роботи з AdBoard Pro</p>
        </div>
        
        <div class="row g-4">
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="100">
                <div class="card h-100 border-0 shadow-sm hover-scale">
                    <div class="card-body text-center p-4">
                        <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                             style="width: 80px; height: 80px;">
                            <i class="fas fa-rocket fa-2x text-white"></i>
                        </div>
                        <h5 class="card-title fw-bold">Швидкі результати</h5>
                        <p class="card-text text-muted">
                            Ваші оголошення будуть опубліковані миттєво та отримають максимальну видимість
                        </p>
                    </div>
                </div>
            </div>
            
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="200">
                <div class="card h-100 border-0 shadow-sm hover-scale">
                    <div class="card-body text-center p-4">
                        <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                             style="width: 80px; height: 80px;">
                            <i class="fas fa-users fa-2x text-white"></i>
                        </div>
                        <h5 class="card-title fw-bold">Широка аудиторія</h5>
                        <p class="card-text text-muted">
                            Тисячі активних користувачів щодня шукають саме те, що ви пропонуєте
                        </p>
                    </div>
                </div>
            </div>
            
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="300">
                <div class="card h-100 border-0 shadow-sm hover-scale">
                    <div class="card-body text-center p-4">
                        <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                             style="width: 80px; height: 80px;">
                            <i class="fas fa-shield-alt fa-2x text-white"></i>
                        </div>
                        <h5 class="card-title fw-bold">Безпека та надійність</h5>
                        <p class="card-text text-muted">
                            Всі оголошення проходять модерацію, гарантуючи високу якість контенту
                        </p>
                    </div>
                </div>
            </div>
            
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="400">
                <div class="card h-100 border-0 shadow-sm hover-scale">
                    <div class="card-body text-center p-4">
                        <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                             style="width: 80px; height: 80px;">
                            <i class="fas fa-chart-line fa-2x text-white"></i>
                        </div>
                        <h5 class="card-title fw-bold">Аналітика та статистика</h5>
                        <p class="card-text text-muted">
                            Детальна статистика переглядів та взаємодій з вашими оголошеннями
                        </p>
                    </div>
                </div>
            </div>
            
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="500">
                <div class="card h-100 border-0 shadow-sm hover-scale">
                    <div class="card-body text-center p-4">
                        <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                             style="width: 80px; height: 80px;">
                            <i class="fas fa-mobile-alt fa-2x text-white"></i>
                        </div>
                        <h5 class="card-title fw-bold">Мобільна оптимізація</h5>
                        <p class="card-text text-muted">
                            Сайт відмінно працює на всіх пристроях - від смартфонів до комп'ютерів
                        </p>
                    </div>
                </div>
            </div>
            
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="600">
                <div class="card h-100 border-0 shadow-sm hover-scale">
                    <div class="card-body text-center p-4">
                        <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                             style="width: 80px; height: 80px;">
                            <i class="fas fa-headset fa-2x text-white"></i>
                        </div>
                        <h5 class="card-title fw-bold">Підтримка 24/7</h5>
                        <p class="card-text text-muted">
                            Наша команда підтримки завжди готова допомогти вам у будь-який час
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Services Section -->
<section class="services-section py-5 bg-light">
    <div class="container">
        <div class="text-center mb-5" data-aos="fade-up">
            <h2 class="display-5 fw-bold mb-3">Наші послуги</h2>
            <p class="lead text-muted">Комплексні рішення для вашого бізнесу</p>
        </div>
        
        <div class="row g-4">
            <div class="col-lg-6" data-aos="fade-right">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            <div class="gradient-bg rounded-circle d-flex align-items-center justify-content-center me-3" 
                                 style="width: 60px; height: 60px;">
                                <i class="fas fa-bullhorn fa-lg text-white"></i>
                            </div>
                            <h4 class="mb-0 fw-bold">Дошка оголошень</h4>
                        </div>
                        <p class="text-muted mb-3">
                            Безкоштовне розміщення оголошень у різних категоріях. 
                            Простий інтерфейс, швидка публікація, широка аудиторія.
                        </p>
                        <ul class="list-unstyled">
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>Безлімітна кількість оголошень</li>
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>Завантаження фото та відео</li>
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>Геолокація та карти</li>
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>Соціальні мережі інтеграція</li>
                        </ul>
                        <a href="<?php echo SITE_URL; ?>/ads" class="btn gradient-bg text-white">
                            Переглянути оголошення <i class="fas fa-arrow-right ms-2"></i>
                        </a>
                    </div>
                </div>
            </div>
            
            <div class="col-lg-6" data-aos="fade-left">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            <div class="gradient-bg rounded-circle d-flex align-items-center justify-content-center me-3" 
                                 style="width: 60px; height: 60px;">
                                <i class="fas fa-ad fa-lg text-white"></i>
                            </div>
                            <h4 class="mb-0 fw-bold">Рекламні послуги</h4>
                        </div>
                        <p class="text-muted mb-3">
                            Професійне просування вашого бізнесу. Створення та запуск 
                            ефективних рекламних кампаній у різних каналах.
                        </p>
                        <ul class="list-unstyled">
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>Контекстна реклама</li>
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>SMM просування</li>
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>SEO оптимізація</li>
                            <li class="mb-2"><i class="fas fa-check text-success me-2"></i>Медійна реклама</li>
                        </ul>
                        <a href="<?php echo SITE_URL; ?>/services" class="btn gradient-bg text-white">
                            Дізнатися більше <i class="fas fa-arrow-right ms-2"></i>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Statistics Section -->
<section class="stats-section py-5">
    <div class="container">
        <div class="row text-center g-4">
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="100">
                <div class="stat-item">
                    <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                         style="width: 80px; height: 80px;">
                        <i class="fas fa-users fa-2x text-white"></i>
                    </div>
                    <h3 class="fw-bold text-gradient counter" data-target="25000">0</h3>
                    <p class="text-muted">Активних користувачів</p>
                </div>
            </div>
            
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="200">
                <div class="stat-item">
                    <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                         style="width: 80px; height: 80px;">
                        <i class="fas fa-bullhorn fa-2x text-white"></i>
                    </div>
                    <h3 class="fw-bold text-gradient counter" data-target="150000">0</h3>
                    <p class="text-muted">Оголошень опубліковано</p>
                </div>
            </div>
            
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="300">
                <div class="stat-item">
                    <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                         style="width: 80px; height: 80px;">
                        <i class="fas fa-handshake fa-2x text-white"></i>
                    </div>
                    <h3 class="fw-bold text-gradient counter" data-target="89000">0</h3>
                    <p class="text-muted">Успішних угод</p>
                </div>
            </div>
            
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="400">
                <div class="stat-item">
                    <div class="gradient-bg rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                         style="width: 80px; height: 80px;">
                        <i class="fas fa-star fa-2x text-white"></i>
                    </div>
                    <h3 class="fw-bold text-gradient">4.9</h3>
                    <p class="text-muted">Рейтинг сервісу</p>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- CTA Section -->
<section class="cta-section py-5">
    <div class="gradient-bg">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-8" data-aos="fade-right">
                    <h2 class="display-6 fw-bold text-white mb-3">
                        Готові розпочати?
                    </h2>
                    <p class="lead text-white opacity-75 mb-0">
                        Приєднуйтесь до тисяч задоволених клієнтів та розвивайте свій бізнес разом з нами!
                    </p>
                </div>
                <div class="col-lg-4 text-lg-end" data-aos="fade-left">
                    <a href="<?php echo SITE_URL; ?>/register" class="btn btn-light btn-lg hover-scale me-3">
                        <i class="fas fa-user-plus me-2"></i>Реєстрація
                    </a>
                    <a href="<?php echo SITE_URL; ?>/contact" class="btn btn-outline-light btn-lg hover-scale">
                        <i class="fas fa-envelope me-2"></i>Контакти
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>

<script>
// Анімація лічильників
$(document).ready(function() {
    $('.counter').each(function() {
        const $this = $(this);
        const target = parseInt($this.data('target'));
        
        $({ counter: 0 }).animate({
            counter: target
        }, {
            duration: 2000,
            easing: 'easeOutExpo',
            step: function() {
                $this.text(Math.ceil(this.counter).toLocaleString());
            },
            complete: function() {
                $this.text(target.toLocaleString());
            }
        });
    });
});
</script>
