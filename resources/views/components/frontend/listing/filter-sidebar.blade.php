            ...
            <div>
                ...
                <div x-show="expanded" x-collapse x-cloak>
                    ...
                    maxtrigger()" class="relative price-range p-4 max-w-xl w-full">
                        <div>
                            <input type="range" step="100" wire:model="minPrice" :min="min" :max="max"
                                x-on:input="mintrigger" x-model="minprice"
                                class="absolute pointer-events-none appearance-none z-20 h-2 w-full opacity-0 cursor-pointer">

                            <input type="range" wire:model="maxPrice" step="100" :min="min" :max="max"
                        ...
                        <div
                            class="flex gap-1.5 justify-center items-center text-gray-600 dark:text-gray-100 body-md-400 mt-5">
        ...
        <div x-data="{ expanded: true }" class="relative overflow-hidden  mb-4">
            <div class="cursor-pointer group filter-item">
                ...
                <div x-show="expanded" x-collapse x-cloak class="pb-4">
                    <div
                        class="flex justify-between items-center gap-3 hover:bg-gray-50 hover:dark:bg-gray-600 tc-checkbox-wrap">
                    ...
                    <div wire:key="category-{{ $category['id'] }}" x-data="{ open: false }">
                        <div
                            class="flex pe-1.5 justify-between items-center hover:bg-gray-50 hover:dark:bg-gray-600 tc-checkbox-wrap">
                            <label class="tc-radio cursor-pointer flex-grow px-4">
                                <input wire:model="selectedCategory" id="category{{ $category['id'] }}" type="radio"
                                    value="{{ $category['slug'] }}">
                            ...
                            <button
                                class="h-10 px-2.5 transition-all duration-300 dark:text-gray-100 text-gray-700 hover:bg-gray-100 hover:text-primary-500"
                                @click="open = !open">
                                <template x-if="!open">
                                ...
                                <div
                                    ...
                                    <div class="tc-radio">
                                        <input type="radio" id="subcategory{{ $subcategory['id'] }}"
                                            wire:model="selectedSubcategory" value="{{ $subcategory['slug'] }}">
                ...
                <div x-show="expanded" x-collapse x-cloak>
                    <div class="px-4 pb-4">
                        ...
                        <ul class="flex flex-col gap-4">
                            ...
                            <h3 class="text-gray-900 dark:text-gray-200 body-md-500 mb-2">{{ __('custom_field') }}</h3>
                            @foreach ($customfields as $field)
                            {{-- $this->customfields = $customField->values->toArray(); --}}
                            @if ($field['filterable'] == 1)
                            <div wire:key="field-{{ $field['id'] }}" x-data="{ open: false }">
                                <div class="flex justify-between items-center">
                                    <label class="tc-radio cursor-pointer flex-grow ">
                                        <label for="custom-field-{{ $field['id'] }}"
                                            class="block dark:text-gray-100 w-full">
                                            {{ $field['translations']['category'][$locale] ?? $field['name'] }}
                                        </label>
                                    </label>
                                    <button
                                        class="h-10 px-2.5 transition-all duration-300 dark:text-gray-100 text-gray-700 hover:bg-gray-100 hover:text-primary-500"
                                ...
                                <div x-show="open" x-cloak>
                                    ...
                                    <div wire:key="custom-field-{{ $subField['id'] }}">
                                        <div
                                            class="flex justify-between items-center gap-3 py-2 pl-9 pr-4 hover:bg-gray-50 tc-checkbox-wrap">
                                            <div class="tc-radio">
                                                <input type="checkbox" id="custom-field-{{ $subField['id'] }}"
                                                    wire:model="selectedCustomFieldValues.{{ $subField['id'] }}"
                                                    value="{{ $subField['value'] }}">
                                                <label for="custom-field-{{ $subField['id'] }}">
                                                    {{ $subField['translations'][$locale] ?? $subField['value'] }}
                                                </label>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="cursor-pointer group filter-item">
                ...
                <div x-show="expanded" x-collapse x-cloak>
                    <div class="px-4 pb-4">
                        <ul class="flex flex-col gap-4">
                            <li class="tc-check-radio">
                                <input type="radio" name="posted-within" wire:model="dateRange" value="1" id="24hrs">
                                <label for="24hrs">Last 24 hrs</label>
                            </li>
                            <li class="tc-check-radio">
                                <input type="radio" wire:model="dateRange" value="7" name="posted-within" id="7days">
                                <label for="7days">Last 7 days</label>
                            </li>
                            <li class="tc-check-radio">
                                <input type="radio" wire:model="dateRange" value="30" name="posted-within" id="30days">
