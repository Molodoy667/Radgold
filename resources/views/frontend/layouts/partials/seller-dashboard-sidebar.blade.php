...
<div
    ...
    <div class="px-6 flex items-center gap-4">
        ...
        <div class="flex flex-col gap-1">
            <h3 class="heading-06 text-gray-900"> {{ $user->name }}</h3>
            <p class="body-md-400 text-gray-600">{{ __('member_since') }} {{ $user->created_at->format('M, Y') }}</p>
    ...
    @if ($user->bio)
        <div class="p-6 pt-0 border-b flex flex-col gap-2 border-b-gray-100">
            <h6 class="heading-07 text-gray-900">{{ __('about_seller') }}</h6>
            <p class="body-md-400 text-gray-700">{{ $user->bio }}</p>
        </div>
    @endif

    <div class="px-[1.5rem]  space-y-[0.75rem] w-full">
        <h6 class="heading-07 text-gray-900">{{ __('contact_information') }}</h6>
        <div class="divide-y divide-gray-100">
            ...
            @if ($user->phone)
                <div class="flex items-start space-x-[0.75rem] pb-[1rem]">
                    ...
                    <div class="space-y-[0.13rem]">
                        <small class="body-xs-500 text-gray-500 leading-[1rem] uppercase">{{ __('phone_number') }}</small>
                        <p class="body-md-500 leading-[1.5rem] text-gray-900">{{ $user->phone }}</p>
                    </div>
                </div>
            @endif
            ...
            @if ($user->email)
                <div class="flex items-start space-x-[0.75rem] py-[1rem]">
                    ...
                    <div class="space-y-[0.13rem]">
                        <small class="body-xs-500 text-gray-500 leading-[1rem] uppercase">{{ __('email_address')}}</small>
                        <p class="body-md-500 leading-[1.5rem] text-gray-900">{{ $user->email }}</p>
                    </div>
                </div>
            @endif
            ...
            @if ($user->address)
                <div class="flex items-start space-x-[0.75rem] pt-[1rem]">
                    ...
                    <div class="space-y-[0.13rem]">
                        <small class="body-xs-500 text-gray-500 leading-[1rem] uppercase">{{ __('address')}}</small>
                        <p class="body-md-500 leading-[1.5rem] text-gray-900">{{ $user->address }}</p>
                    </div>
                </div>
            @endif
    ...
    @if (auth('user')->check() && $user->id != auth('user')->id())
        ...
        <form action="{{ route('block.user') }}" method="POST" class=" ml-3" style="margin-left:20px" id="blockUserForm">
            @csrf
            <input type="hidden" name="user_id" value="{{ $user->id }}">
            <button type="submit" class="inline-flex justify-center items-center">
                <span class="icon mr-1">
                    <x-frontend.icons.block stroke="#FF0000" />
                </span>
                {{ __('Block User') }}
            </button>
        </form>
    ...
    @if ( !auth('user')->check() && $user->id != auth('user')->id())
        <a href="{{ route('users.login') }}" class="login_required flex items-center text-gray-400 mody-md-500 gap-3 w-full px-6">
            <x-frontend.icons.warning />
            {{ __('report_seller') }}
        </a>
    @endif
