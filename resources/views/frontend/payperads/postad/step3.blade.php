...
<div x-data="{ whatsapp: false }">
    <form action="{{ route('frontend.payper.ads.post.step3.store') }}" method="POST">
        @csrf
        <div class="sm:p-8 p-4 flex flex-col gap-8">
            ...
            <div class="promotions-wrapper">
                ...
                <div class="flex items-center mb-2">
                    ...
                    <p class="text-sm">
                        (<a href="{{ route('frontend.promotions') }}" class="pl-1 underline text-blue-500"
                            target="_blank">{{ __('know_more_about_promotion') }}</a>):
                    </p>
                </div>
                ...
                <div class="promotion-item mt-3 mb-3">
                    ...
                    <label for="featured"
                        class="cursor-pointer flex promote-ad border-2 border-transparent px-4 py-3 rounded-md bg-gray-50 w-full">
                        <input @checked(old('featured', $ad?->featured ?? '') == 1) type="checkbox" class="hidden"
                        name="featured" id="featured" value="1">
                        <div class="flex align-center">
                            <img class="h-14" src="{{ asset('frontend/images/promotions-img/featured-promote.svg') }}"
                                alt="">
                            @php
                            $plan = loadSetting()::query()->select('id', 'per_ads_active', 'per_ads_price',
                            'highlight_ads_price', 'highlight_ads_duration', 'featured_ads_price',
                            'featured_ads_duration', 'top_ads_price', 'top_ads_duration', 'bump_ads_price',
                            'bump_ads_duration', 'urgent_ads_price', 'urgent_ads_duration')->first();

                            @endphp
                            <div class="text-left grow ml-3">
                                <h3 class="heading-07">{{ __('featured_listing') }} <b>
                                        ( {{ __('price') }}
                                        {{ currentCurrencySymbol() }} {{ currencyDollarExchange($plan->featured_ads_price)
                                        }} )
                                    </b></h3>
                                ...
                                <p class="leading-6">{{ $user_plan_data->featured_limit }} {{ __('featured_listing') }}
                                    {{ __('remaining') }}.</p>
                            </div>
                        </div>
                    </label>
                ...
                <div class="promotion-item mt-3 mb-3">
                    ...
                    <label for="urgent"
                        class="cursor-pointer flex promote-ad border-2 border-transparent px-4 py-3 rounded-md bg-gray-50 w-full">
                        <input @checked(old('urgent', $ad?->urgent ?? '') == 1) type="checkbox" class="hidden"
                        name="urgent" id="urgent" value="1">
                        <div class="flex align-center">
                            <img class="h-14" src="{{ asset('frontend/images/promotions-img/urgent-promote.svg') }}"
                                alt="">
                            <div class="text-left grow ml-3">
                                <h3 class="heading-07">{{ __('urgent_listing') }}   <b> 
                                    ( {{ __('price') }}
                                    {{ currentCurrencySymbol() }} {{ currencyDollarExchange($plan->urgent_ads_price)
                                    }} ) 
                                </b></h3>
                                <p class="leading-6">{{ __('select_this_promotion_to_make_listing_urgent') }}
                                    {{ __('for') }}
                                    {{ $plan->urgent_ads_duration < 1 ? __('lifetime') : $plan->urgent_ads_duration }}
                                        @if ($plan->urgent_durgent_ads_durationuration == 1)
                                        {{ __('day') }}.
                                        @else
                                        {{ __('days') }}.
                                        @endif
                                </p>
                                <p class="leading-6">{{ $user_plan_data->urgent_ads_duration }} {{ __('urgent_listing')
                                    }} {{ __('remaining') }}.</p>
                            </div>
                        </div>
                    </label>
                ...
                <div class="promotion-item mt-3 mb-3">
                    ...
                    <label for="highlight"
                        class="cursor-pointer flex promote-ad border-2 border-transparent px-4 py-3 rounded-md bg-gray-50 w-full">
                        <input @checked(old('highlight', $ad?->highlight ?? '') == 1) type="checkbox" class="hidden"
                        name="highlight" id="highlight" value="1">
                        <div class="flex align-center">
                            <img class="h-14" src="{{ asset('frontend/images/promotions-img/highlight-promote.svg') }}"
                                alt="">
                            <div class="text-left grow ml-3">
                                <h3 class="heading-07">{{ __('highlight_listing') }} <b> 
                                    ( {{ __('price') }}
                                    {{ currentCurrencySymbol() }} {{ currencyDollarExchange($plan->highlight_ads_price)
                                    }} ) 
                                </b></h3>
                                <p class="leading-6">
                                ...
                                <p class="leading-6">{{ $user_plan_data->highlight_limit }} {{ __('highlight_listing')
                                    }} {{ __('remaining') }}.</p>
                            </div>
                        </div>
                    </label>
                ...
                <div class="promotion-item mt-3 mb-3">
                    ...
                    <label for="top"
                        class="cursor-pointer flex promote-ad border-2 border-transparent px-4 py-3 rounded-md bg-gray-50 w-full">
                        <input @checked(old('top', $ad?->top ?? '') == 1) type="checkbox" class="hidden" name="top"
                        id="top" value="1">
                        <div class="flex align-center">
                            <img class="h-14" src="{{ asset('frontend/images/promotions-img/top-promote.svg') }}"
                                alt="">
                            <div class="text-left grow ml-3">
                                <h3 class="heading-07">{{ __('top_listing') }} <b> 
                                    ( {{ __('price') }}
                                    {{ currentCurrencySymbol() }} {{ currencyDollarExchange($plan->top_ads_price)
                                    }} ) 
                                </b></h3>
                                ...
                                <p class="leading-6">{{ $user_plan_data->top_limit }} {{ __('top_listing') }} {{
                                    __('remaining') }}.</p>
                            </div>
                        </div>
                    </label>
                ...
                <div class="promotion-item mt-3 mb-3">
                    ...
                    <label for="bump_up"
                        class="cursor-pointer flex promote-ad border-2 border-transparent px-4 py-3 rounded-md bg-gray-50 w-full">
                        <input @checked(old('bump_up', $ad?->bump_up ?? '') == 1) type="checkbox" class="hidden"
                        name="bump_up" id="bump_up" value="1">
                        <div class="flex align-center">
                            <img class="h-14" src="{{ asset('frontend/images/promotions-img/bumpup-promote.svg') }}"
                                alt="">
                            <div class="text-left grow ml-3">
                                <h3 class="heading-07">{{ __('bump_up_listing') }} <b> 
                                    ( {{ __('price') }}
                                    {{ currentCurrencySymbol() }} {{ currencyDollarExchange($plan->bump_ads_price)
                                    }} ) 
                                </b></h3>
                                <p class="leading-6">
                                ...
                                <p class="leading-6">{{ $user_plan_data->bump_up_limit }} {{ __('bump_up_listing') }} {{
                                    __('remaining') }}.</p>
                            </div>
                        </div>
                    </label>
        ...
        <div class="post-footer">
            ...
            <div>
                <a href="{{ route('frontend.post.step2.back') }}"
                    onclick="return confirm('{{ __('are_you_sure_to_go_back') }}')" class="py-3 px-5">{{ __('back')
                    }}</a>
